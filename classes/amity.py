...
class Amity(object):
    ...
    def create_room(self, room_type, *room_names):
        ...
        for room_name in room_names:
            if room_name in self.offices.keys() or room_name in self.living.keys():
                return "Room {} already exists".format(room_name)
            elif room_name.isdigit():
                return "Room name can not have just numbers"
            elif room_name.isalnum():
                if room_type == "OFFICE":
                    new_office = Office(room_name, "Office", 6)
                    self.offices[new_office.room_name] = []
                    print(term.bggreen + "Office {} has been successfully created".format(room_name) + term.off)
                elif room_type == "LIVING":
                    new_living = LivingSpace(room_name, "Living Space", 4)
                    self.living[new_living.room_name] = []
                    print(
    ...
    def add_person(self, first_name, last_name, person_type, wants_accommodation=None):
        ...
        if len(self.offices) == 0:
            return term.red + "You need to add at least one office to add a person" + term.off
    ...
    def assign_office(self, new_person):
        ...
        available_offices = {}
        for office_name, occupants in self.offices.items():
            if len(self.offices[office_name]) < 6:
                available_offices[office_name] = occupants
        if available_offices == {}:
            ...
            choice_office = random.choice(list(available_offices.keys()))
    ...
    def assign_living(self, new_fellow):
        ...
        available_living = {}
        for living_name, occupants in self.living.items():
            if len(self.living[living_name]) < 4:
                available_living[living_name] = occupants
        if available_living == {}:
            ...
            choice_living = random.choice(list(available_living.keys()))
    ...
    def reallocate_person(self, person_id, new_room_name):
        ...
        if person_id in (person.person_id for person in self.people):
            for person in self.people:
                ...
                if new_room_name in self.offices.keys():
                    for office, occupants in self.offices.items():
                        if person in occupants:
                            ...
                            return "{} did not have an office before and therefore can not be reallocated".format(person.person_name)

                elif new_room_name in self.living.keys():
                    for living, occupants in self.living.items():
                        if person in occupants:
                            ...
                            return "{} did not have a living space before and therefore can not be reallocated".format(
                                person.person_name )
    ...
    def load_people(self, file_name):
        ...
        if len(self.offices) == 0:
            return "You need to add atleast one office to add a person"
        else:
            try:
                ...
                if os.stat( 'static/' + file_name ).st_size == 0:
                    return "{} is an empty file".format( file_name )
                else:
    ...
    def print_allocations(self, file_name=None):
        ...
        if len(self.offices) == 0 and len(self.living) == 0:
    ...
    def allocations_view(self):
        ...
        for room_name in self.living.keys():
    ...
    def print_room(self, room_name):
        ...
        """
        if room_name in self.offices.keys():
            if self.offices[room_name]:
                print("\n" + room_name + "\n-----------------------\n")
                roomies = ""
                for person in self.offices[room_name]:
        ...
        elif room_name in self.living.keys():
            if self.living[room_name]:
                print("\n" + room_name + "\n---------------------\n")
                roomies = ""
                for person in self.living[room_name]:
    ...
    def save_state(self, db_name):
        if not self.offices:
            ...
            session = DBSession()
            amity_data = ( self.people, self.offices, self.living, self.unallocated_office,
                          self.unaallocated_living)
    ...
    def load_state(self, db_name):
        if os.path.isfile(db_name):
            ...
            self.people, self.offices, self.living, self.unallocated_office, self.unaallocated_living = app_data
