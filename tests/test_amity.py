...
from classes.room import Office, Room
...
class TestAmity( unittest.TestCase ):
    ...
    def test_create_room_multiple_living(self):
        ...
        self.assertEqual( {"Impala": [], "Eland": []},
                          self.amity.living,
                          msg="Unable to create multiple living spaces!" )

    @patch.object(Amity, 'create_room', side_effect= [amity_mock.create_room("Office", "Africa" ),
                                                      amity_mock.create_room("office", "Asia" ),
                                                      amity_mock.create_room("OFFICE", "America" ),
                                                      amity_mock.create_room("OfFicE", "Europe")])
    def test_create_room_different_case(self, create_room_mock):

        create_room_mock()
        create_room_mock()
        create_room_mock()
        create_room_mock()
        self.assertEqual(create_room_mock.call_count, 4)
        # amity_mock.create_room.return_value = {"Africa": [], "Asia": [], "America": [], "Europe": []}
        # output = amity_mock.create_room()
        # self.assertIsNotNone( output )
        self.assertEqual( {"Africa": [], "Asia": [], "America": [], "Europe": []},
                          amity_mock.offices,
    ...
    def test_create_room_name_has_special_characters(self):
        self.assertIn( "Afr!ca has not been added because it has special characters",
                       self.amity.create_room( "Office", "Afr!ca" ) )

    def test_create_room_name_is_number(self):
        self.assertIn( "Room name can not have just numbers",
                       self.amity.create_room( "Office", "1" ) )
    ...
    def test_create_room_repeated_name_same_type(self):
        self.amity.create_room( "Office", "Africa" )
        self.assertEqual( {"Africa": []}, self.amity.offices )
        self.amity.create_room( "Office", "Africa" )
        self.assertEqual( {"Africa": []}, self.amity.offices )

    def test_create_room_repeated_name_diff_type(self):
        self.amity.create_room( "Living", "Africa" )
        len1 = len( self.amity.living )
        self.amity.create_room( "Living", "Africa" )
        len2 = len( self.amity.living )
    ...
    def test_office_subclass(self):
        self.assertIsInstance( Office, type( Room ) )
    ...
    def test_add_person_no_rooms(self):
        self.assertEqual( term.red + "You need to add at least one office to add a person" + term.off,
    ...
    def test_reallocate_fellow_to_new_living(self):
        ...
        self.amity.create_room( "Living", "Brown" )
        self.amity.add_person( "Christine", "Nagadya", "Fellow", "Y" )
        self.amity.create_room( "Living", "Blue" )
        self.amity.reallocate_person( 1, "Blue" )
        self.assertIn( self.amity.people[-1], self.amity.living["Blue"] )
    ...
    def test_reallocate_person_without_living_to_living(self):
        self.amity.create_room( "Office", "Africa" )
        self.amity.create_room( "Living", "Brown" )
        self.amity.add_person( "Christine", "Nagadya", "Fellow" )
        self.assertEqual( "Christine Nagadya did not have a living space before and therefore can not be reallocated",
                          self.amity.reallocate_person( 1, "Brown" ),
                          "Should not reallocate person who wasnt previously allocated!" )
    ...
    def test_reallocate_person_office_full(self):
        ...
        self.amity.reallocate_person( 1, "Asia" ),
        self.assertNotIn( self.amity.people[-13],
                          self.amity.offices["Asia"],
